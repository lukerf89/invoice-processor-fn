...
def test_onehundred80_processing():
    ...
    with open(
        "test_invoices/ONEHUNDRED80-7-1-2025-1T25194476NCHR_docai_output.json", "r"
    ) as f:
    ...
    if vendor_type == "OneHundred80":
        ...
        rows = extract_line_items_from_entities(
            document, invoice_date, vendor, purchase_order or invoice_number
        )
    else:
    ...
    for i, row in enumerate(rows, 1):
        ...
        print(
            f"  Row {i}: {row[4][:60]}{'...' if len(row[4]) > 60 else ''} | ${row[5]} | Qty: {row[6]}"
        )
