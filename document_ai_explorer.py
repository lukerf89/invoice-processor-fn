    ...
    for i, entity in enumerate(document.entities, 1):
        ...
        if hasattr(entity, 'properties') and entity.properties:
            ...
            for prop in entity.properties:
                print(f"      {prop.type_}: {prop.mention_text} (conf: {prop.confidence:.3f})")
    ...
    for page in document.pages:
        if hasattr(page, 'form_fields') and page.form_fields:
            ...
            for i, field in enumerate(page.form_fields, 1):
                field_name = get_text_from_layout(field.field_name, document.text) if field.field_name else "Unknown"
                field_value = get_text_from_layout(field.field_value, document.text) if field.field_value else "No value"
            ...
            for table_num, table in enumerate(page.tables, 1):
                ...
                if table_data:
                    ...
                    for row_num, row in enumerate(table_data):
                        row_str = " | ".join(
                            str(cell)[:30].ljust(col_widths[i]) if i < len(col_widths) else str(cell)[:30]
